...
func (cq *CredentialQuery) QueryUsers() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, credential.UsersTable, credential.UsersPrimaryKey...),
		)
...
func (cq *CredentialQuery) WithUsers(opts ...func(*UserQuery)) *CredentialQuery {
	query := (&UserClient{config: cq.config}).Query()
...
//		GroupBy(credential.FieldCredentialType).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Credential.Query().
//		Select(credential.FieldCredentialType).
//		Scan(ctx, &v)
...
func (cq *CredentialQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Credential, error) {
	...
	if query := cq.withUsers; query != nil {
		if err := cq.loadUsers(ctx, query, nodes,
			func(n *Credential) { n.Edges.Users = []*User{} },
			func(n *Credential, e *User) { n.Edges.Users = append(n.Edges.Users, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CredentialQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Credential, init func(*Credential), assign func(*Credential, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*Credential)
	nids := make(map[int64]map[*Credential]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(credential.UsersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(credential.UsersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(credential.UsersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(credential.UsersPrimaryKey[1]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Credential]struct{}{byID[outValue]: {}}
